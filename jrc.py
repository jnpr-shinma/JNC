...
class ClassGenerator(object):
    ...
    def generate_routeclass(self, module):
        ...
        if self.ctx.opts.verbose:
            print('Generating REST API Routes class "' + self.filename + '"...')
        ...
        self.routing = [' ' * 4 + "val " + camelize(module.arg) + "RestApiRouting = compressResponseIfRequested(new RefFactoryMagnet()) {"]
        ...
        if (len(res) > 0):
            ...
            self.routing[len(self.routing)-1] = ' ' * 6 + '}'
            self.routing.append(' ' * 4 + '}')
    ...
    def generate_implclass(self):
        ...
        body.append(indent + "private def refreshIndex: Future[String] = {")
        body.append(indent + "  val pipeline: HttpRequest => Future[String] = (")
        body.append(indent + "     sendReceive")
        body.append(indent + "       ~> unmarshal[String]")
        body.append(indent + "     )")
        body.append(indent + '  pipeline(ElasticSearchUtil.post(module + "/_refresh", null))')
        body.append(indent + "}")
        body.append(indent)
        ...
        body.append(body_indent + "        }")
        body.append(body_indent + "        else {")
        body.append(body_indent + "          Some("+self.n2+".toJson(false))")
        body.append(body_indent + '        }')
        body.append(body_indent + '      }')
        body.append(body_indent + '    })')
        body.append(body_indent + '}')
    ...
    def write_rpc_to_file(self):
        if hasattr(self, "rpc_class") == True:
            ...
            if self.ctx.opts.debug or self.ctx.opts.verbose:
                print('Generating "' + impl_filename + '"...')
