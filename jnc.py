...
def get_uses_path(stmt):
    ...
    while parent is not None:
        ...
        parent = get_parent(stmt)
        sub_packages.appendleft(camelize(stmt.arg))
    return '/'.join(sub_packages)

def get_dependency_stmt(stmt, name):
    ...
    def as_list(self):
        ...
        if stmt.keyword in {'container', 'list'}:
            if hasattr(stmt, 'i_uses'):
                ...
                package = get_package(stmt, self.ctx)
                yang_java_type = package + '.' + normalize(stmt.arg)
            res.append('<yang_java_type>' + yang_java_type + '</yang_java_type>')
            ...
            if field is not None:
                ...
                if (not self.ctx.opts.import_on_demand
                    ...
                    if hasattr(ch, 'i_uses'):
                        import_ = '.'.join([get_uses_package(ch, self.ctx), ch_arg])
                    else:
